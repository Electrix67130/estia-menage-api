...
| **Dashboard** (admin web) | `estia-menage-dashboard` | Next.js (App Router) | tag git `[0-9]*` → CI SSH → VPS | dernier tag **0.1.77** |
...
- **Calendrier** (dashboard + mobile, presta ET admin) : **vue mois** ; chaque **séjour** = une **barre colorée multi-jours** allant du check-in au check-out (regroupe ménage + check-in + check-out d'une réservation via `external_event_uid` ; arrivée = `date_prevue − stay_nights`), colorée par logement, en **couloirs** par semaine. **Demi-journées aux extrémités** : un check-out (matin) et un check-in (après-midi) le même jour tiennent **côte à côte** (turnover, pas de superposition) ; un **ménage manuel** prend toute la journée sauf s'il partage sa date avec un check-in → il se scinde aussi (matin). **Dashboard** : infos check-in/out (icône + heure) sur les jours d'arrivée/départ + **clic ciblé** (jour d'arrivée → check-in, départ → check-out, milieu → ménage). **Mobile** : barres visuelles, tap une case → détail du jour (liste des prestations, navigables).
