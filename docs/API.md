...
| POST | `/menages/:id/restore` | « Remettre » une prestation auto retirée (admin) — `sync_ignored=false` + `status=a_venir`. Réponse `200 { restored:true }`. |
| POST | `/menages/:id/arrival` | Pointage arrivée (prestataire assigné) — body `{ photo_url, lat, lng, arrived_at?, traveler_rating?, has_degradation?, degradation_note?, degradation_photos? }`. Photo géolocalisée obligatoire. `traveler_rating` 1-5. Si `has_degradation`, `degradation_photos: [{ url, thumbnail_url?, file_size?, mime_type? }]` est enregistré dans `photo` avec `is_degradation=true`. **`arrived_at`** (ISO) optionnel : horodatage réel du pointage fourni par le client (file d'attente hors ligne) ; absent → `now`. Nouveaux champs optionnels (rétro-compat). |
| POST | `/menages/:id/departure` | Pointage départ (prestataire assigné) — body `{ photo_url, lat, lng, departed_at? }`. **`departed_at`** (ISO) optionnel : horodatage réel du départ (offline) ; absent → `now`. `date_realisation` = jour réel du départ. |
